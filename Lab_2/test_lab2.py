...
class TestLab2(unittest.TestCase):
    ...
    def test_averages(self):
        ...
        correctAvgWaitTime = (0, 5.50, 5.0, 7, 4.5, 5.5, 6.25, 4.75)
        correctAvgRespTime = (0, 0.75, 1.5, 2.75, 3.25, 3.25, 4, 4.75)

        self.assertTrue(self.make, msg='make failed')
        for x in range(1, 7):
            try:
                cl_result = subprocess.check_output(('./rr', fileName, str(x)), timeout=10).decode()
                ...
                self.fail(f"test_averages failed: Test timed out for quantum length {x}. Possible infinite loop detected.")
    ...
    def test_arrival_and_requeue(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            for x in range(1, 7):
                try:
                    cl_result = subprocess.check_output(('./rr', f.name, str(x)), timeout=10).decode()
                    lines = cl_result.split('\n')
                    testAvgWaitTime = float(lines[0].split(':')[1])
                    testAvgRespTime = float(lines[1].split(':')[1])
                    ...
                    self.fail(f"test_arrival_and_requeue failed: timed out for quantum length {x} with temporary file. Possible infinite loop detected.")